...
class MyClient(discord.Client):
		...
		if key:
			...
			if "verified" in captcha_verification_message.content and captcha_verification_message.author.id == self.owo['id']:
				...
				await self.send_webhooks(
					...
					description = f"{self.emoji['arrow']}**Answer:** {key}\n{self.emoji['arrow']}**Continue To Work**",
					color = discord.Colour.random(),
					thumnail = image
				)
	...
	async def on_message(self, message):
		...
		#Startup after solve captchas successfully
		...
		#Commands
		if self.command['mode'] and message.author.id in self.command['owner_id']:
			#Help
	...
	async def on_message_edit(self, before, after):
		...
		#Join owo's giveaway
		if self.join_owo_giveaway and after.embeds and after.id not in self.selfbot['giveaway_entered'] and after.author.id == self.owo['id']:
	...
	async def check_distorted_animal(self):
		if self.use_gem_when_distorted_animals_are_available and self.selfbot['work_status'] and self.owo['status'] and self.selfbot['distorted_animals_time'] - time.time() <= 0:
			await self.discord['channel'].typing()
			...
			async for message in self.discord['channel'].history(limit = 10):
				if message.author.id == self.owo['id'] and (await self.get_messages(message, "are available") or await self.get_messages(message, "not available")):
					distorted_animals_message = message
					break

...
class MyClient(discord.Client):
	def __init__(self, token, *args, **kwargs):
		...
		self.twocaptcha_image_captcha = TwoCaptcha(**{
			"server": "2captcha.com",
			"apiKey": self.image_captcha['service']['twocaptcha']['key'],
			"defaultTimeout": 300,
			"pollingInterval": 5
		})

		self.twocaptcha_hcaptcha = TwoCaptcha(**{
			"server": "2captcha.com",
			"apiKey": self.hcaptcha['service']['twocaptcha']['key'],
			"defaultTimeout": 300,
			"pollingInterval": 5
		})

		self.capmonster_image_captcha = ImageToTextTask(self.image_captcha['service']['twocaptcha']['key'])

		self.capmonster_hcaptcha = HCaptchaTask(self.hcaptcha['service']['twocaptcha']['key'])
	...
	async def on_ready(self):
		if self.selfbot['on_ready']:
			...
			if self.command['mode']:
				webhook = f"{self.arrow}Send **`help`** To **Get Command Menu**\n" + webhook
	...
	async def solve_image_captcha(self, image, captcha, lenghth):
		...
		if self.image_captcha['service']['twocaptcha']['mode'] and not key:
			try:
				...
				key = self.twocaptcha_solver.normal(captcha, numeric=2, minLen=lenghth, maxLen=lenghth, phrase=0, caseSensitive=0, calc=0, lang="en")
				key = key['code']
				...
				pass
		#CapMonster
		if self.image_captcha['service']['capmonster']['mode'] and not key:
			try:
				task_id = self.capmonster_solver_image_captcha.create_task(base64_encoded_image = captcha)
				key = self.capmonster_solver_image_captcha.join_task_result(task_id)
				key = key.get("text")
			except Exception as e:
				await self.send_webhooks(
					content = self.selfbot['mentioner'],
		...
		if key:
			...
			if "verified" in captcha_verification_message.content and captcha_verification_message.author.id == self.owo['id']:
				...
				await self.send_webhooks(
					...
					description = f"{self.arrow}**Answer:** {key}\n{self.arrow}**Continue To Work**",
					color = discord.Colour.random(),
					thumnail = image
				)
	...
	async def solve_hcaptcha(self):
		...
		key = None
		#TwoCaptcha
		if self.hcaptcha['service']['twocaptcha']['mode'] and not key:
			try:
				balance = self.twocaptcha_solver.balance()
				print(f"{await self.intro()}{color.blue}[INFO]{color.reset} {color.bold}Your TwoCaptcha API Currently Have{color.reset} {color.green}{balance}${color.reset}")
				...
				pass
		#CapMonster
		if self.hcaptcha['service']['capmonster']['mode'] and not key:
			try:
	...
	async def on_message(self, message):
		#Change Channel When Someone Meation
		...
		if self.checking['captcha_appear'] and not self.selfbot['work_status'] and "I have verified that you are human!" in message.content and message.channel.id == self.owo['dm_channel_id'] and message.author.id == self.owo['id']:
			self.amounts['captcha'] += 1
		...
		if self.command['mode'] and (message.author.id in self.command['owner_id'] or message.author.id  == self.discord['user_id']):
			...
			if message.content.lower() == "help":
				print(f"{await self.intro()}{color.blue}[INFO]{color.reset} {color.bold}Send Command Menu{color.reset} {color.gray}Via Webhook{color.reset}")
				await self.send_webhooks(
			...
			if message.content.lower() == "stat":
				...
				await self.send_webhooks(
					title = f"STAT 📊",
					description = f"I Worked **<t:{self.selfbot['turn_on_time']}:R>** With:\n{self.arrow}Sent **__{self.amounts['command']}__ Commands**\n{self.arrow}Solved **__{self.amounts['captcha']}__ Captchas**\n{self.arrow}Claimed Huntbot **__{self.amounts['huntbot']}__ Times**\n{self.arrow}Used Gem **__{self.amounts['gem']}__ Times**\n{self.arrow}Gambled **__{self.amounts['gamble']}__ Cowoncy**\n{self.arrow}Changed Channel **__{self.amounts['change_channel']}__ Times**\n{self.arrow}Slept **__{self.amounts['sleep']}__ Times**",
					color = discord.Colour.random()
				)
		...
		if self.selfbot['work_status'] and self.owo['status'] and after.channel.id == self.discord['channel_id'] and after.author.id == self.owo['id']:
			...
			if self.gamble['slot']['mode'] and str(self.discord['user_nickname']) in after.content:
				...
				if "won nothing" in after.content:
					...
					self.amounts['gamble'] -= self.current_gamble_bet['slot']
				...
				if "<:cherry:417475705178161162> <:cherry:417475705178161162> <:cherry:417475705178161162>" in after.content:
					...
					self.amounts['gamble'] += self.current_gamble_bet['slot'] * 2
					self.current_gamble_bet['slot'] = int(self.gamble['slot']['bet'])
				#Won x4
				if "<:cowoncy:417475705912426496> <:cowoncy:417475705912426496> <:cowoncy:417475705912426496>" in after.content:
					...
					self.amounts['gamble'] += self.current_gamble_bet['slot'] * 3
					self.current_gamble_bet['slot'] = int(self.gamble['slot']['bet'])
				#Won x10
				if "<:o_:417475705899843604> <:w_:417475705920684053> <:o_:417475705899843604>" in after.content:
					...
					self.amounts['gamble'] += self.current_gamble_bet['slot'] * 9
					self.current_gamble_bet['slot'] = int(self.gamble['slot']['bet'])
			...
			if self.gamble['coinflip']['mode'] and str(self.discord['user_nickname']) in after.content:
				...
				if "you lost" in after.content:
					...
					self.amounts['gamble'] -= self.current_gamble_bet['coinflip']
		...
		if self.use_gem_when_glitch_are_available and self.selfbot['work_status'] and self.owo['status'] and self.selfbot['glitch_time'] - time.time() <= 0:
			...
			if glitch_message:
				if "are available" in glitch_message.content:
					...
					print(f"{await self.intro()}{color.blue}[INFO]{color.reset} {color.bold}Distorted Animals Are Available For{color.reset} {color.green}{str(datetime.timedelta(seconds = int(glitch_end)))} Seconds{color.reset}")
	...
	async def sell_sac_animal(self):
		if self.animals['mode'] and self.selfbot['work_status'] and self.owo['status']:
			...
			print(f"{await self.intro()}{color.yellow}[SEND] {self.owo['prefix']}{self.animals['type']} {self.animals['rank']}{color.reset}")
			self.amounts['command'] += 1
			...
			while not self.checking['blackjack_end']:
				...
				if message:
					if "in progress" in message.embeds[0].footer.text or "resuming previous" in message.embeds[0].footer.text:
						...
						self.amounts['gamble'] += self.current_gamble_bet['blackjack']
						self.current_gamble_bet['blackjack'] = int(self.gamble['blackjack']['bet'])
